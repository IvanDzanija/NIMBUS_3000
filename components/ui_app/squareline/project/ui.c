...
static lv_timer_t *ui_alarm_watch_timer = NULL;

static void ui_alarm_watch_timer_cb(lv_timer_t *timer)
{
    static unsigned int last_alarm_version = 0;
    ...
    version = ui_get_alarm_trigger_version();
    if (!ui_is_alarm_ringing() || version == last_alarm_version) {
        return;
    }

    last_alarm_version = version;
    disp = lv_disp_get_default();
    active_screen = disp ? lv_disp_get_scr_act(disp) : NULL;

    if (active_screen != ui_Alarm_Ringing_Scr) {
        lv_scr_load_anim(ui_Alarm_Ringing_Scr, LV_SCR_LOAD_ANIM_FADE_ON, 250, 0, false);
    }
}
...
ui_Alarm_Scr_screen_init();
ui_Alarm_Ringing_Scr_screen_init();
ui_Color_Scr_screen_init();
ui_Settings_Scr_screen_init();
ui_Whats_Down_screen_init();
ui_MamaKontakt_screen_init();
ui_TataKontakt_screen_init();
ui_PrimljenePoruke_screen_init();
ui_Zivotinje_screen_init();
...
{if (ui_alarm_watch_timer) {
lv_timer_del(ui_alarm_watch_timer);
ui_alarm_watch_timer = NULL;
}
ui_Home_Scr_screen_destroy();
ui_Alarm_Scr_screen_destroy();
ui_Alarm_Ringing_Scr_screen_destroy();
ui_Color_Scr_screen_destroy();
ui_Settings_Scr_screen_destroy();
ui_Whats_Down_screen_destroy();
ui_MamaKontakt_screen_destroy();
ui_TataKontakt_screen_destroy();
ui_PrimljenePoruke_screen_destroy();
ui_Zivotinje_screen_destroy();
