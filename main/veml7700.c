...
static uint32_t veml7700_get_maximum_lux(void);
static int veml7700_get_gain_index(uint8_t gain);
static int veml7700_get_it_index(uint8_t integration_time);
static int indexOf(uint8_t elm, const uint8_t *ar, uint8_t len);
...
static struct veml7700_config veml7700_get_default_config()
{
	...
	configuration.persistence = VEML7700_PERS_1;
	configuration.interrupt_enable = false;
	configuration.shutdown = false;
	configuration.resolution = 0.0f;
	configuration.maximum_lux = 0;
...
static esp_err_t veml7700_optimize_configuration(veml7700_handle_t dev, double *lux)
{
	if (ceil(*lux) >= veml7700_get_current_maximum_lux(dev)) {	// Decrease resolution
		...
		if (dev->configuration.maximum_lux == veml7700_get_maximum_lux()) {
...
static esp_err_t veml7700_send_config(veml7700_handle_t dev)
{
	uint16_t config_data = ( 
		...
		(dev->configuration.persistence << 4) |
...
{
	if ((dev == NULL) || (configuration == NULL)) {
...
{
	esp_err_t read_result = veml7700_read_als_lux(dev, lux);
...
{
	esp_err_t read_result = veml7700_read_white_lux(dev, lux);
	if (read_result != ESP_OK) {
		return read_result;
	}
